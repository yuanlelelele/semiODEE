...
class FormatDataSet(object):
    ...
    def span_with_sample(self, data, batch):
        ...
        invalid_cpt_idx = [0] * self.args.span_max_cpt_num
        ...
        for exm in data:
            ...
            sent_span_cpt_masks.extend([invalid_cpt_idx for _ in range(self.max_span_length)])
            for anchor_idx, (anchor_spans, anchor_cpt_ids) in enumerate(zip(exm["sent_spans"], exm["sent_cpt_ids"])):
                for offset in range(self.max_span_length):
                    ...
                    if cur_span not in anchor_spans:
                        ...
                        sent_span_cpt_masks.append(invalid_cpt_idx)
                        continue
            ...
            batch["span_cpt_masks"].append(sent_span_cpt_masks + [invalid_cpt_idx for _ in range(max_span_num - len(sent_span_cpt_masks))])
    ...
    def new_dummy_batch(self):
        ...
        for step in range(self.batch_num):
            ...
            if self.args.add_mlm_object and self.mode != 'Testing':
                tmp_masked_idx = [exm.mask_idx for exm in tmp_batch_data]
                tmp_masked_label = [exm.mask_label for exm in tmp_batch_data]

                batch["target_idx"], batch["target_label"], batch["target_mask"] = self.padding_target_mask_labels(tmp_masked_idx, tmp_masked_label)
                batch["target_idx"] = torch.LongTensor([ids for ids in batch["target_idx"]])
                batch["target_label"] = torch.LongTensor([ids for ids in batch["target_label"]])
                batch["target_mask"] = torch.LongTensor([ids for ids in batch["target_mask"]])
